...
namespace UpgradeHelpers.Gui.Controls
{
	...
	public static partial class ControlHelper
	{
		...
		/// <summary>
		/// Given the name of a VB control, it return the corresponding System.Windows.Form control
		/// </summary>
		/// <param name="vbControlName">The name of the control type in VB6</param>
		/// <returns>A corresponding control in System.Windows.Form for the given vbControlName</returns>
		private static Control MapVBControlName(string vbControlName)
		{
			switch (vbControlName.ToLower())
			{
				case "checkbox":
					return new CheckBox();
				case "combobox":
		...
		/// <summary>
		/// Given the fullControlName like "Lib.ControlTypeName", it calls the Add function with Lib as projectName and ControlTypeName as controlName
		/// </summary>
		/// <param name="controls">Collection of controls</param>
		/// <param name="assembly">Actual assembly</param>
		/// <param name="fullControlName">The complete name of the control type to be created, must match "Lib.ControlTypeName" pattern</param>
		/// <param name="controlID">Id of the control (generally used as the name of the control)</param>
		/// <param name="Parent">The parent control where to add the new instance control</param>
		/// <param name="createControl">Flag to indicate if the Control needs to be added or just created (Default true)</param>
		/// <returns></returns>
		public static Control Add(this Control.ControlCollection controls, Assembly assembly, string fullControlName, string controlID, Control parent = null, bool createControl = true)
		{
			string projectName, controlName;
			Regex regex = new Regex(@"(\w*)\.(\w*)");
			Match match = regex.Match(fullControlName);
			if (match.Success)
			{
				// match.Groups[0] is the whole match
				projectName = match.Groups[1].Value;
				controlName = match.Groups[2].Value;
				return Add(controls, assembly, projectName, controlName, controlID, parent, createControl);
			}

			throw new NotSupportedException("Parameters do not match the expected values");
		}

		/// <summary>
		/// Adds dynamically by reflection (according to a name passed as parameter) a new control to the ControlCollection
		/// </summary>
		/// <param name="controls">Collection of controls</param>
		/// <param name="assembly">Actual assembly</param>
		/// <param name="projectName">Name of the project</param>
		/// <param name="controlName">Name of the control to insert</param>
		/// <param name="controlID">Id of the control (generally used as the name of the control)</param>
		/// <param name="Parent">The parent control where to add the new instance control</param>
		/// <param name="createControl">Flag to indicate if the Control needs to be added or just created (Default true)</param>
		/// <returns>A new Control</returns>
		public static Control Add(this Control.ControlCollection controls, Assembly assembly, string projectName, string controlName, string controlID, Control parent = null, bool createControl = true)
		{
			try
			{
				...
				Assembly objAssembly = Assembly.LoadFrom(path);
				Type type = objAssembly.GetType(string.Format("{0}.{1}", projectName, controlName));
				Control newInstance = null;

				//Required because in some cases the name of the project has some differences with the dll
				if (type == null)
				{
					type = objAssembly.GetType(string.Format("{0}.{1}", assemblyName, controlName));
				}

				if (type != null)
				{
					newInstance = Activator.CreateInstance(type) as Control;
				}

				if (newInstance == null && projectName.ToLower().Equals("vb"))
				{
					newInstance = MapVBControlName(controlName);
				}

				if (newInstance != null)
				{
					newInstance.Name = controlID;
					if (createControl)
					{
						if (parent != null && parent.Controls != null)
						{
							parent.Controls.Add(newInstance);
						}
						else
						{
		...
		/// <summary>
		/// Get the parent Container if is UserControl or Form
		/// </summary>
		/// <param name="mControl">Instance of the Control</param>
		/// <returns>Control object that allows access to the control properties of the container</returns>
		public static Control GetAmbient(this Control mControl)
		{
			if (mControl.Parent == null)
			{
				return null;
			}
			else if (mControl.Parent is UserControl || mControl.Parent is Form)
			{
				return mControl.Parent;
			}
			else
			{
				return GetAmbient(mControl.Parent);
			}
		}
		...
		/// <summary>
		/// This method should only be used with control arrays. Returns the number of not null elements in the given control array.
		/// VB6 allows elements with bigger indexes than array.length. But the "Count" method returns the actual number of elements.
		/// </summary>
		/// <param name="array">The given control array</param>
		/// <returns>The number of not null elements in the control array</returns>
		public static int ControlCount(this Array array)
		{
			int count = 0;
		...
		public class ControlGraphics
		{
			...
			public void Circle(int x, int y, double radius, Color color)
			{
				...
				try
                {
                    ...
                    float height = (float)radius * 2;
                    radius = radius * 1.108;
					x -= (int)radius / 2;
					y -= (int)radius / 2;
                    g.DrawEllipse(pen, newX, newY, width, height);
                    CurrentX = x;
					CurrentY = y;
				}
				finally
