    ...
    public static class DateTimePickerHelper
    {
        static DateTimePickerHelper()
        {
            try
            {
                ...
                //might fail because it is an init only field
		...
		{
			if (dt.ShowCheckBox && dt.Checked)
			{
				return dt.Value;
			}
			else if (!dt.ShowCheckBox)
			{
				return dt.Value;
			}
			else
		...
		public static bool ValueIsNull(this DateTimePicker dt)
        {
            return dt.GetValue() == DBNull.Value;
        }
