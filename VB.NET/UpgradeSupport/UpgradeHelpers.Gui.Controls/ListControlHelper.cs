...
namespace UpgradeHelpers.Gui.Controls
{
    ...
    public partial class ListControlHelper : Component, IExtenderProvider, ISupportInitialize
    {
        ...
        {
            return extendee is ListControl;
        }
        ...
        {
            if ((_itemsData.ContainsKey(lstControl)) && (_itemsData[lstControl].Length > index && index >= 0))
            {
        ...
        private void SetItemData(ComboBox lstControl, int[] itemsData)
        {
            ...
            if (itemsData != null)
            {
                if (DesignMode || _onInitialization)
                {
                    ...
                    for (int i = 0; (i < lstControl.Items.Count) && (i < itemsData.Length); i++)
                        lstControl.SetItemData(i, itemsData[i]);
                }
                else
                {
                    for (int i = 0; (i < lstControl.Items.Count) && (i < itemsData.Length); i++)
    ...
    /// </summary>
    public static class ListControlExtenders
    {
        private class Item
        {
            public int itemdata;
        ...
        private static void AddItem(this ListBox lstControl, string value, int index)
        {
            ...
            {
                //Item was not added to the control.
            }
        }
		...
		/// <summary>
		/// Gets the Columns value of a ListBox control.
		/// </summary>
		/// <param name="lstControl">The listbox control instance</param>
		/// <returns>The value of the ColumnsWidth property</returns>
		public static int GetColumnWidth(this ListBox lstControl)
		{
			try
			{
				int result = Convert.ToInt32(Math.Round(decimal.Divide(lstControl.Width, lstControl.ColumnWidth)));
				return result;
			}
			catch (Exception)
			{
		...
		#region ListViewItem
		//Hexadecimal numbers of the Windows Messages
		private const int LVM_GETITEM = 0x1005;
        private const int LVM_SETITEM = 0x1006;
        //This bit indicates that the 'Ghosted' property is going to be set/retrieved
        private const int LVIS_CUT = 0x4;
        //This bit indicates that the LVITEM state is going to be set/retrieved
        ...
        private static extern int SendMessageA(IntPtr hwnd, int wMsg, int wParam, ref LVITEM pitem);
        ...
        public static bool GetGhosted(this ListViewItem instance)
        {
            try
            {
                LVITEM lvItem = new LVITEM();
                ...
                lvItem.stateMask = LVIS_CUT;
                lvItem.state = 0;
                //This message returns the result in the LVM_GETITEM state attribute.
                int response = SendMessageA(instance.ListView.Handle, LVM_GETITEM, 0, ref lvItem);
                //If the returned bit is on the property is set.
                return lvItem.state == LVIS_CUT;
            }
            catch
            {
                return false;
