...
namespace UpgradeHelpers.Gui.Controls
{
    ...
    public class ControlArrayHelper
    {
        ...
        public static void LoadControl(Control controlContainer, String controlName, int index)
        {
            ...
            if ((fieldInfo != null) && fieldInfo.FieldType.IsArray)
            {
                ...
                if (baseControl != null)
                {
                    ...
                    controlArray.SetValue(CloneObject(baseControl, index, controlContainer), index);
        ...
        /// <param name="container">The actual controller to get delegates </param>
        /// <returns>A copy or null.</returns>
        private static Object CloneObject(Object baseControl, int index, Control container = null)
        {
            Object result = null;
            if (baseControl != null)
            {
                ...
                AddCloneToContainer(baseControl, result, index);
                ...
                {
                    Delegate[] eventDelegates = ContainerHelper.GetEventSubscribers(baseControl, eventInfo.Name, container);
                    //The event in the new control will be bound to the same delegates of the base control
                    if (eventDelegates != null)
                    {
                        foreach (Delegate del in eventDelegates)
        ...
        private static void AddCloneToContainer(object baseObject, object clone, int index)
        {
            ...
            {
                ToolStripItem bsObj = (ToolStripItem)baseObject;

                var bsObjIndex = ((ToolStripItem)baseObject).Owner.Items.IndexOf(bsObj);
                ...
                {
                    ((ToolStripItem)baseObject).Owner.Items.Insert(bsObjIndex + index, (ToolStripItem)clone);
                }
            }
        }
