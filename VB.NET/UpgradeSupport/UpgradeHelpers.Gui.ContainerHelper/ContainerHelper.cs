...
namespace UpgradeHelpers.Gui
{
    ...
    public class ContainerHelper
    {
        ...
        /// <param name="controlContainer">Controller to get delegates</param>
        /// <returns>Null if no delegates or event were found.</returns>
        public static Delegate[] GetEventSubscribers(object target, string eventName, Control controlContainer = null)
        {
            ...
            while (targetType != null)
            {
                ...
                // IMPORTANT: To have delegates is necessary to get the name of the method, to make that we suppose that method name is the concatenation between control name and event name
                EventInfo eventInfo = targetType.GetEvent(eventName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance);
                if(eventInfo != null && controlContainer != null)
                {
					string methodName = "";

					// Look for the method associated with event
					try
					{
						var targetControl = (target as Control);
						if (targetControl != null)
							methodName = getMethodName(targetControl.Name, eventName);

						var methodInfo = controlContainer.GetType().GetMethod(methodName,
							BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static);
						if (methodInfo != null)
						{
							Delegate handler = Delegate.CreateDelegate(eventInfo.EventHandlerType, controlContainer, methodInfo);
							AddListOfEventsToChache(target, eventName, handler.GetInvocationList());
							return EventSubscribersCache[target][eventName];
						}
					}
					finally
					{
						// There is the possibility of not considering a case,
						// so it must be necessary to have the code inside the try block
					}
                }
        ...
        /// <summary>
        /// Gets the name of method (concat control name and event name)
        /// </summary>
        /// <param name="targetName">Name of the control</param>
        /// <param name="eventName">Name of the event</param>
        /// <returns>Name of method</returns>
        private static string getMethodName(string targetName, string eventName)
        {
            targetName = targetName.Substring(targetName.IndexOf("_") + 1);
            targetName = targetName.Substring(0, targetName.IndexOf("_"));

            string resultName = targetName + "_" + eventName;
    ...
    public class NestedControlEnumerator : IEnumerator<Control>, IEnumerable<Control>
    {
        ...
        IEnumerator<Control> IEnumerable<Control>.GetEnumerator()
        {
			_controlEnumerator = _control.Controls.GetEnumerator();
			return this;
        }
