...
namespace UpgradeHelpers.Helpers
{
	...
	public class XArrayHelper : DataTable
	{
		...
		public XArrayHelper RedimXArray(int[] lengths, int[] lowerBounds)
		{
			...
			{
				for (int colIndex = Columns.Count - 1; colIndex >= lengths[1] + 1; colIndex--)
				{
			...
			{
				for (int rowIndex = Rows.Count - 1; rowIndex >= lengths[0] + 1; rowIndex--)
				{
		...
		public Object Find(Object value)
		{
            ...
            if(value != null)
            {
                int[] lowerBounds = { GetLowerBound(0), GetLowerBound(1) };
                int[] upperBounds = { GetUpperBound(0), GetUpperBound(1) };
                for (int row = lowerBounds[0]; row <= upperBounds[0]; row++)
                {
                    for (int col = lowerBounds[1]; col <= upperBounds[1]; col++)
                    {
		...
		public Object Find(Object value, int rowStart, int column)
		{
			long index = -1;
            if(value != null)
            {
                int rowUpperBound = GetUpperBound(0);
                for (int row = rowStart; row <= rowUpperBound; row++)
                {
                    if(value.Equals(GetValue(row, column)))
                    {
                        index = row;
                        break;
                    }
