	...
	public class ArraysHelper
	{
		...
		public static TA CastArray<TA>(Array srcArray) where TA : class
		{
			...
			if (finalResult == null)
			{
				try
				{
					//The next two lines would be equivalent to calling Linq extensions methods "Cast" and "ToArray" with
					//generic parameter<itemType> but since itemType is a System.Type, we can't use it like that:
					//   finalResult = srcArray.Cast<itemType>().ToArray<itemType>() as TA;
					var value = typeof(System.Linq.Enumerable).GetMethod("Cast").MakeGenericMethod(itemType).Invoke(srcArray, new object[] { srcArray });
					finalResult = typeof(System.Linq.Enumerable).GetMethod("ToArray").MakeGenericMethod(itemType).Invoke(value, new object[] { value }) as TA;
				}
				catch { }
				finally
				{
		...
		public static object DeepCopy(object objectToCopy)
		{
			...
			try
			{
				using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream())
				{
					...
					System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(objectToCopy.GetType());
					serializer.Serialize(memoryStream, objectToCopy);
...
#endif
			{
				if (objectToCopy is ICloneable)
				{
					return ((ICloneable)objectToCopy).Clone();
				}
				throw;
			}
		}
