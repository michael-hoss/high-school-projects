...
namespace UpgradeHelpers.Helpers
{
	...
	public class StringsHelper
	{
        ...
        */
        ...
        public static int BinaryCompare(string String1, string String2)
        {
		...
		/// <summary>
		/// Implementation to work like MidB in assignments
		/// </summary>
		...
		/// <returns></returns>
		public static string MidBAssignment(string str, int start, int nBytes, string val, Encoding encoding)
		{
			if (str == null)
			{
				return null;
			...
			if (nBytes < 0)
			{
				throw new InvalidOperationException("The nBytes parameter cannot be a negative value");
			}
			...
			{
				throw new InvalidOperationException("The start parameter cannot be greater than str in bytes length");
			}

			byte[] valBytes = encoding.GetBytes(val);
			nBytes = Math.Min(nBytes, strBytes.Length - Math.Min(strBytes.Length, start)); // Avoiding buffer over-runs
			byte[] segmentBytes = new byte[strBytes.Length];
			...
			for (int i = 0; i < strBytes.Length; i++)
			{
				if (i == start)
				{
					for (int j = 0; j < nBytes && j < valBytes.Length; j++)
					{
						segmentBytes[i++] = valBytes[j];
					}
					i--;
				}
				else
		...
		public static string MidBAssignment(string str, int start, int nBytes, string val)
		{
			return MidBAssignment(str, start, nBytes, val, Encoding.Unicode);
		}

		public static string MidBAssignment(string str, int start, string val)
		{
			return MidBAssignment(str, start, Encoding.Unicode.GetByteCount(str) - start, val, Encoding.Unicode);
		}
	}
}
