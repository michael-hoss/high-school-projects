...
namespace UpgradeHelpers.Helpers
{
	...
	{
		private static object InteractionCallByName(object obj, string propName, Microsoft.VisualBasic.CallType callType, object[] indexesOrArgs, object value)
		{
			object result = null;
			...
			switch (callType)
			{
				case CallType.Get:
#if !NET_STANDARD_APP
					result = Interaction.CallByName(obj, propName, CallType.Get, indexesOrArgs);
#else
					// Originally the second parameter was obj.GetType(), but since it generates an Exception, null must be assigned
					result = Microsoft.VisualBasic.CompilerServices.NewLateBinding.LateGet(obj, null, propName, indexesOrArgs, null, null, null);
#endif
					break;
				case CallType.Let:
				case CallType.Set:
#if !NET_STANDARD_APP
					if (indexesOrArgs != null && indexesOrArgs.Length > 0)
					{
						indexesOrArgs = new object[] { indexesOrArgs, value };
					}
					else
					{
						indexesOrArgs = new object[] { value };
					}
					result = Interaction.CallByName(obj, propName, callType, indexesOrArgs);
#else
					if (indexesOrArgs != null && indexesOrArgs.Length > 0)
					{
						var oldIndex = indexesOrArgs.Length;
						Array.Resize(ref indexesOrArgs, oldIndex + 1);
						indexesOrArgs[oldIndex] = value;
					}
					else
					{
						indexesOrArgs = new object[]{ value };
					}
					Microsoft.VisualBasic.CompilerServices.NewLateBinding.LateSet(obj, obj.GetType(), propName, indexesOrArgs, null, null, false, false, callType);
#endif
					break;
				case CallType.Method:
#if !NET_STANDARD_APP
					result = Interaction.CallByName(obj, propName, CallType.Method, indexesOrArgs);
#else
					result = Microsoft.VisualBasic.CompilerServices.NewLateBinding.LateCall(obj, obj.GetType(), propName, indexesOrArgs, null, null, null, false);
#endif
					break;
		...
		public static T GetMember<T>(object obj, string propName, params object[] indexes)
		{
			...
			result = InteractionCallByName(obj, propName, CallType.Get, indexes, null);
		...
		public static void SetMember(object obj, string propName, object value, params object[] indexes)
		{
			...
			InteractionCallByName(obj, propName, CallType.Set, indexes, value);
		}
		#endregion
		...
		public static void LetMember(object obj, string propName, object value, params object[] indexes)
		{
			propName = GetDotNetMemberName(obj.GetType(), propName, true);
			value = GetValueForced(obj, propName, value);
			InteractionCallByName(obj, propName, CallType.Let, indexes, value);
		}
		...
		public static T Invoke<T>(object obj, string memberName, object[] parameters)
		{
			...
			object result = InteractionCallByName(obj, memberName, pInfo != null ? CallType.Get : CallType.Method, parameters, null);
			return (T)GetValueForcedToType(result, typeof(T));
		}
        ...
        /// <summary>
        /// Verifies if the object a is less than the object b
        /// </summary>
        /// <param name="a">First object to be compared</param>
        /// <param name="b">Second object to be compared</param>
        /// <returns>true if the object a is less than the object b, false otherwise.</returns>
		public static bool IsLessThan(object a, object b)
		{
			...
			if (expandedA is decimal && expandedB is decimal)
				return GetPrimitiveValue<decimal>(expandedA) < (decimal)GetPrimitiveValue<decimal>(expandedB);
			...
			{
				throw new ArgumentException(string.Format("ReflectionHelper could not compare two objects of types '{0}' and '{1}'",
					expandedA.GetType().ToString(), expandedB.GetType().ToString()));
			}
		}

        /// <summary>
        /// Verifies if the object a is greater than the object b
        /// </summary>
        /// <param name="a">First object to be compared</param>
        /// <param name="b">Second object to be compared</param>
        /// <returns>true if the object a is greater than the object b, false otherwise.</returns>
        ...
        /// <summary>
        /// Verifies if the object a is less or equal than the object b
        /// </summary>
        /// <param name="a">First object to be compared</param>
        /// <param name="b">Second object to be compared</param>
        /// <returns>true if the object a is less or equal than the object b, false otherwise.</returns>
        ...
        /// <summary>
        /// Verifies if the object a is greater or equal than the object b
        /// </summary>
        /// <param name="a">First object to be compared</param>
        /// <param name="b">Second object to be compared</param>
        /// <returns>true if the object a is greater or equal than the object b, false otherwise.</returns>
		...
		private static DateTime GetValueForcedToDate(object propertyValue)
		{
			...
			{
				DateTime dValue;
				string strValue = propertyValue != null ? propertyValue.ToString().Trim() : null;
				return DateTime.TryParse(strValue, out dValue) ? dValue : string.IsNullOrEmpty(strValue) ? default(DateTime) : DateTime.FromOADate(Double.Parse(strValue));
			}
#if WINFORMS
			if (sourceType.FullName == "System.Drawing.Color")
		...
		{
			return CursorHelper.GetCursorValueForcedToInteger(GetValueForcedToCursor(propertyValue));
		}
		...
		public static System.Windows.Forms.Cursor GetValueForcedToCursor(object propertyValue)
		{
			...
			return CursorHelper.GetValueForcedToCursor(GetValueForcedToInt(propertyValue));
		}
#endif
